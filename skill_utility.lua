...
local brightOn, generatorOn, playerOn = false, false, false
local autoPerfect, genDelay = false, 0
local checkWasVisible = false
local parryOn, parryRadius = false, 10
local parryMode = "Button"
local parryCircle
...
local debugLines = {}
local debugLiveText = "Auto Perfect: OFF | Gen Delay: 0ms\nParry: OFF"
...
debugTitle.Text = "SKILL / PARRY DEBUG"
...
-- Gen Delay slider
local genDelayContainer = Instance.new("Frame")
genDelayContainer.Position = UDim2.new(0, 10, 0, 114)
genDelayContainer.Size = UDim2.new(1, -20, 0, 29)
genDelayContainer.BackgroundColor3 = Color3.fromRGB(51, 57, 78)
genDelayContainer.BorderSizePixel = 0
genDelayContainer.Parent = panel
Instance.new("UICorner", genDelayContainer).CornerRadius = UDim.new(0, 7)

local genDelayFill = Instance.new("Frame")
genDelayFill.Size = UDim2.new(0, 0, 1, 0)
genDelayFill.BackgroundColor3 = Color3.fromRGB(90, 125, 220)
genDelayFill.BackgroundTransparency = 0.4
genDelayFill.BorderSizePixel = 0
genDelayFill.Parent = genDelayContainer
Instance.new("UICorner", genDelayFill).CornerRadius = UDim.new(0, 7)

local genDelayLabel = Instance.new("TextLabel")
genDelayLabel.Size = UDim2.new(1, 0, 1, 0)
genDelayLabel.BackgroundTransparency = 1
genDelayLabel.Font = Enum.Font.SourceSansSemibold
genDelayLabel.TextSize = 13
genDelayLabel.TextColor3 = Color3.fromRGB(230, 235, 250)
genDelayLabel.Text = "Gen Delay: 0ms"
genDelayLabel.ZIndex = 2
genDelayLabel.Parent = genDelayContainer

local genDelayTrack = Instance.new("TextButton")
genDelayTrack.Size = UDim2.new(1, 0, 1, 0)
genDelayTrack.BackgroundTransparency = 1
genDelayTrack.Text = ""
genDelayTrack.ZIndex = 3
genDelayTrack.Parent = genDelayContainer

local autoPerfectButton = makeButton("Auto Perfect Generator: OFF", 148)
...
-- Gen Delay slider click
connect(genDelayTrack.MouseButton1Down, function(x)
	local ratio = math.clamp(x / genDelayTrack.AbsoluteSize.X, 0, 1)
	genDelay = math.floor(ratio * 8 + 0.5) * 50
	genDelay = math.clamp(genDelay, 0, 400)
	genDelayLabel.Text = "Gen Delay: " .. genDelay .. "ms"
	genDelayFill.Size = UDim2.new(genDelay / 400, 0, 1, 0)
end)

-- Auto Perfect Generator toggle
connect(autoPerfectButton.MouseButton1Click, function()
	autoPerfect = not autoPerfect
	setToggle(autoPerfectButton, "Auto Perfect Generator", autoPerfect)
	debugPanel.Visible = autoPerfect or parryOn
	checkWasVisible = false
	addDebug("AUTO PERFECT " .. (autoPerfect and "enabled" or "disabled"))
end)
...
connect(RunService.RenderStepped, function(dt)
	...
	if autoPerfect then
		...
		if gui and gui.Enabled then
			...
			local checkVisible = check and check.Visible
			if checkVisible and not checkWasVisible then
				local delay = math.random() * 0.2 + 0.15 + genDelay / 1000
				task.spawn(function()
					task.wait(delay)
				...
				addDebug("PERFECT trigger delay=" .. string.format("%.0fms", delay * 1000))
			end
			checkWasVisible = checkVisible
		else
			checkWasVisible = false
		end
		setDebugLive(string.format(
			"Auto Perfect: ON | Gen Delay: %dms\nCheck: %s",
			genDelay, checkWasVisible and "visible" or "hidden"
		))
	else
		setDebugLive(string.format(
			"Auto Perfect: OFF\nParry: %s | Mode: %s | Radius: %d",
			parryOn and "ON" or "OFF", parryMode, parryRadius
		))
	end
...
-- Lookup ParryEvent at startup (non-blocking)
task.spawn(function()
	...
	ParryEvent = dagger and dagger:FindFirstChild("parry")
