--[[
  ...
  Single file, all features inline
...
-- Tabs
local T = {}
T.Main = Window:Tab({ Title = "Main", Icon = "solar:home-2-bold-duotone", IconColor = Color3.fromHex("#83889E"), Border = true })
T.ESP = Window:Tab({ Title = "ESP", Icon = "solar:eye-bold-duotone", IconColor = Color3.fromHex("#83889E"), Border = true })
T.Aimbot = Window:Tab({ Title = "Aimbot", Icon = "solar:target-bold-duotone", IconColor = Color3.fromHex("#83889E"), Border = true })
T.Visuals = Window:Tab({ Title = "Visuals", Icon = "solar:palette-bold-duotone", IconColor = Color3.fromHex("#83889E"), Border = true })
T.Misc = Window:Tab({ Title = "Misc", Icon = "solar:settings-bold-duotone", IconColor = Color3.fromHex("#83889E"), Border = true })
T.HUD = Window:Tab({ Title = "HUD", Icon = "solar:chart-bold-duotone", IconColor = Color3.fromHex("#83889E"), Border = true })
T.Credits = Window:Tab({ Title = "Credits", Icon = "solar:info-circle-bold-duotone", IconColor = Color3.fromHex("#83889E"), Border = true })
...
_G.YH = {
    ...
    WindUI = WindUI, Window = Window, Tabs = T,
    C = { Blue = Color3.fromRGB(0, 120, 255), Red = Color3.fromRGB(255, 50, 50), Green = Color3.fromRGB(0, 255, 100), Yellow = Color3.fromRGB(255, 200, 50), Purple = Color3.fromRGB(160, 0, 255), White = Color3.fromRGB(255, 255, 255), Orange = Color3.fromRGB(255, 150, 50) },
...
-- ============== MAIN TAB ==============
T.Main:Toggle({ Title = "Walkspeed", Callback = function(s) if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.WalkSpeed = s and 50 or 16 end end })
T.Main:Space()
T.Main:Slider({ Title = "Walkspeed Value", Width = 200, Value = { Min = 16, Max = 250, Default = 50 }, Step = 1, Callback = function(v) if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.WalkSpeed = v end end })
T.Main:Space()
T.Main:Dropdown({ Title = "Jump Power", Values = { "50", "75", "100", "150", "200" }, Value = 1, Callback = function(s) if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.JumpPower = tonumber(s) end end })
T.Main:Space()
T.Main:Button({ Title = "Rejoin Server", Callback = function() game:GetService("TeleportService"):Teleport(game.PlaceId, LocalPlayer) end })
T.Main:Space()
...
local genOn = false; local genH = {}; local genL = {}; local hookOn = false; local hookH = {}; local hookL = {}
local palOn = false; local palH = {}; local gateOn = false; local gateH = {}; local gateL = {}; local winOn = false; local winL = {}
...
T.ESP:Toggle({ Title = "Player ESP", Desc = "Highlight + name + distance + status", Callback = function(s) playerESPOn = s; if not s then for _, v in pairs(playerHighlights) do pcall(function() v:Destroy() end) end; ClearTable(playerHighlights); for _, v in pairs(playerLabels) do pcall(function() v.Parent:Destroy() end) end; ClearTable(playerLabels) end end })
T.ESP:Space()
T.ESP:Toggle({ Title = "ESP Box", Callback = function(s) espBoxOn = s; if s then for _, p in pairs(Players:GetPlayers()) do if p ~= LocalPlayer then local box = Drawing.new("Square"); box.Thickness = 2; box.Color = Color3.fromRGB(255, 50, 50); box.Filled = false; box.Visible = false; local nl = Drawing.new("Text"); nl.Center = true; nl.Size = 14; nl.Outline = true; nl.Color = Color3.fromRGB(255, 255, 255); nl.Visible = false; ESPObjs[p] = { Box = box, Name = nl } end end else for _, o in pairs(ESPObjs) do o.Box.Visible = false; o.Name.Visible = false end end end })
T.ESP:Space()
T.ESP:Toggle({ Title = "ESP Line", Callback = function(s) espLineOn = s; if not s then for _, o in pairs(espLineObjs) do o.Visible = false end end end })
T.ESP:Space()
T.ESP:Colorpicker({ Title = "Line Color", Default = espLineColor, Callback = function(c) espLineColor = c end })
T.ESP:Space()
T.ESP:Dropdown({ Title = "Line Mode", Values = { "Single", "All Players" }, Value = 1, Callback = function(s) espLineMode = (s == "All Players") and "All" or "Single" end })
T.ESP:Space()
T.ESP:Dropdown({ Title = "Line Origin", Values = { "Character", "Top Screen" }, Value = 1, Callback = function(s) espLineOrigin = s end })
T.ESP:Space()
T.ESP:Toggle({ Title = "Projectile Arc", Desc = "Trajectory prediction", Callback = function(s) _G.YH.projArcOn = s; if not s and _G.YH.projArcObj then _G.YH.projArcObj.Visible = false end end })
T.ESP:Space()
T.ESP:Toggle({ Title = "Generator ESP", Callback = function(s) genOn = s; if not s then for _, v in pairs(genH) do pcall(function() v:Destroy() end) end; ClearTable(genH); for _, v in pairs(genL) do pcall(function() v.Parent:Destroy() end) end; ClearTable(genL) end end })
T.ESP:Space()
T.ESP:Toggle({ Title = "Hook ESP", Callback = function(s) hookOn = s; if not s then for _, v in pairs(hookH) do pcall(function() v:Destroy() end) end; ClearTable(hookH); for _, v in pairs(hookL) do pcall(function() v.Parent:Destroy() end) end; ClearTable(hookL) end end })
T.ESP:Space()
T.ESP:Toggle({ Title = "Pallet ESP", Callback = function(s) palOn = s; if not s then for _, v in pairs(palH) do pcall(function() v:Destroy() end) end; ClearTable(palH) end end })
T.ESP:Space()
T.ESP:Toggle({ Title = "Gate ESP", Callback = function(s) gateOn = s; if not s then for _, v in pairs(gateH) do pcall(function() v:Destroy() end) end; ClearTable(gateH); for _, v in pairs(gateL) do pcall(function() v.Parent:Destroy() end) end; ClearTable(gateL) end end })
T.ESP:Space()
T.ESP:Toggle({ Title = "Window ESP", Callback = function(s) winOn = s; if not s then for _, v in pairs(winL) do pcall(function() v.Parent:Destroy() end) end; ClearTable(winL) end end })

-- ESP loop
RunService.RenderStepped:Connect(function(dt)
    cacheTimer = cacheTimer + dt
    if cacheTimer >= 2 then cacheTimer = 0; ScanObjects() end
    if espBoxOn then for plr, o in pairs(ESPObjs) do if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then local root = plr.Character.HumanoidRootPart; local pos, on = Camera:WorldToViewportPoint(root.Position); if on then local sz = Vector2.new(2000 / pos.Z, 3000 / pos.Z); o.Box.Size = sz; o.Box.Position = Vector2.new(pos.X - sz.X / 2, pos.Y - sz.Y / 2); o.Box.Visible = true; o.Name.Position = Vector2.new(pos.X, pos.Y - sz.Y / 2 - 16); o.Name.Text = plr.Name; o.Name.Visible = true else o.Box.Visible = false; o.Name.Visible = false end else o.Box.Visible = false; o.Name.Visible = false end end end
    if espLineOn then
        local mp, msp = nil, nil; if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then mp = LocalPlayer.Character.HumanoidRootPart.Position; msp, _ = Camera:WorldToViewportPoint(mp) end
        ...
        local targets = {}; if espLineMode == "Single" then local t = _G.YH.projTarget or GetClosestPlayer(360); if t then table.insert(targets, t) end else for _, p in pairs(Players:GetPlayers()) do if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 then table.insert(targets, p) end end end
    ...
    if playerESPOn then
        for _, plr in pairs(Players:GetPlayers()) do
            ...
            if not playerLabels[plr] or not playerLabels[plr].Parent then local bill = Instance.new("BillboardGui"); bill.Size = UDim2.new(0, 200, 0, 50); bill.StudsOffset = Vector3.new(0, 3, 0); bill.AlwaysOnTop = true; bill.Parent = Camera; local txt = Instance.new("TextLabel"); txt.Size = UDim2.new(1, 0, 1, 0); txt.BackgroundTransparency = 1; txt.Font = Enum.Font.SourceSansSemibold; txt.TextSize = 14; txt.TextStrokeTransparency = 0.3; txt.TextStrokeColor3 = Color3.new(0, 0, 0); txt.TextXAlignment = Enum.TextXAlignment.Center; txt.Parent = bill; playerLabels[plr] = txt end
            local txt = playerLabels[plr]; txt.Parent.Adornee = head; local dist = 0; if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then dist = math.floor((head.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude) end
...
-- ============== AIMBOT TAB ==============
T.Aimbot:Toggle({ Title = "Basic Aimbot", Callback = function(s) _G.YH.aimOn = s end })
T.Aimbot:Space()
T.Aimbot:Slider({ Title = "Smoothness", Width = 200, Value = { Min = 1, Max = 10, Default = 1 }, Step = 1, Callback = function(v) _G.YH.aimSmooth = v end })
T.Aimbot:Space()
T.Aimbot:Slider({ Title = "FOV", Width = 200, Value = { Min = 10, Max = 360, Default = 90 }, Step = 1, Callback = function(v) _G.YH.aimFOV = v end })
T.Aimbot:Space()
T.Aimbot:Toggle({ Title = "Projectile Aimbot", Desc = "For arcing weapons", Callback = function(s) _G.YH.projOn = s end })
T.Aimbot:Space()
T.Aimbot:Slider({ Title = "Projectile Velocity", Width = 200, Value = { Min = 30, Max = 500, Default = 150 }, Step = 5, Callback = function(v) _G.YH.projV = v end })
T.Aimbot:Space()
T.Aimbot:Slider({ Title = "Gravity", Width = 200, Value = { Min = 50, Max = 500, Default = 196.2 }, Step = 1, Callback = function(v) _G.YH.projG = v end })
T.Aimbot:Space()
T.Aimbot:Toggle({ Title = "Lead Prediction", Callback = function(s) _G.YH.projLead = s end })
T.Aimbot:Space()
T.Aimbot:Slider({ Title = "Lead Factor", Width = 200, Value = { Min = 0.5, Max = 3, Default = 1 }, Step = 0.1, Callback = function(v) _G.YH.projLeadFac = v end })
T.Aimbot:Space()
T.Aimbot:Button({ Title = "Lock Target", Color = _G.YH.C.Blue, Callback = function() _G.YH.projTarget = GetClosestPlayer(360) end })
T.Aimbot:Space()
T.Aimbot:Button({ Title = "Unlock Target", Color = _G.YH.C.Red, Callback = function() _G.YH.projTarget = nil end })
...
RunService.RenderStepped:Connect(function()
    if _G.YH.aimOn and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if c and c.Character then local pos = Camera:WorldToViewportPoint(c.Character.HumanoidRootPart.Position); local t = Vector2.new(pos.X, pos.Y); local cur = Vector2.new(Mouse.X, Mouse.Y); local s = t:Lerp(cur, 1 / _G.YH.aimSmooth); mousemoverel(s.X - cur.X, s.Y - cur.Y) end
    end
    if _G.YH.projOn and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") and target.Character:FindFirstChild("Humanoid") and target.Character.Humanoid.Health > 0 then
            local origin = LocalPlayer.Character.HumanoidRootPart.Position; local tPos = target.Character.HumanoidRootPart.Position; local aim = GetAimPoint(origin, tPos, _G.YH.projV, _G.YH.projG)
...
T.Visuals:Space()
T.Visuals:Toggle({ Title = "Custom FOV", Callback = function(s) _G.YH.fovOn = s; Camera.FieldOfView = s and _G.YH.fovVal or _G.YH.origFOV end })
T.Visuals:Space()
...
T.Visuals:Space()
T.Visuals:Slider({ Title = "Exposure", Width = 200, Value = { Min = 0, Max = 100, Default = 50 }, Step = 1, Callback = function(v) _G.YH.skyE = v end })

-- ============== MISC TAB ==============
T.Misc:Toggle({ Title = "Speedhack", Callback = function(s) _G.YH.spdOn = s end })
T.Misc:Space()
T.Misc:Slider({ Title = "Speed Value", Width = 200, Value = { Min = 16, Max = 100, Default = 32 }, Step = 1, Callback = function(v) _G.YH.spdVal = v end })
T.Misc:Space()
T.Misc:Toggle({ Title = "Sprint Speed", Desc = "Faster while holding Shift", Callback = function(s) _G.YH.sprintOn = s end })
T.Misc:Space()
T.Misc:Slider({ Title = "Sprint Boost", Width = 200, Value = { Min = 1.0, Max = 2.0, Default = 1.05 }, Step = 0.05, Callback = function(v) _G.YH.sprintBoost = v end })
T.Misc:Space()
T.Misc:Toggle({ Title = "Noclip", Desc = "Walk through walls", Callback = function(s) _G.YH.noclipOn = s end })
T.Misc:Space()
T.Misc:Toggle({ Title = "Custom Crosshair", Callback = function(s) _G.YH.chOn = s end })
T.Misc:Space()
T.Misc:Slider({ Title = "Crosshair Length", Width = 200, Value = { Min = 5, Max = 30, Default = 10 }, Step = 1, Callback = function(v) _G.YH.chLen = v end })
T.Misc:Space()
T.Misc:Slider({ Title = "Crosshair Width", Width = 200, Value = { Min = 1, Max = 8, Default = 2 }, Step = 1, Callback = function(v) _G.YH.chW = v end })
T.Misc:Space()
T.Misc:Toggle({ Title = "Flashlight", Callback = function(s) _G.YH.flOn = s end })
T.Misc:Space()
T.Misc:Toggle({ Title = "Stretched Res", Desc = "Wider field of view", Callback = function(s) _G.YH.stOn = s end })
T.Misc:Space()
T.Misc:Slider({ Title = "Stretch Amount", Width = 200, Value = { Min = 50, Max = 200, Default = 100 }, Step = 5, Callback = function(v) _G.YH.stVal = v end })
T.Misc:Space()
T.Misc:Button({ Title = "Reset Character", Callback = function() if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then LocalPlayer.Character.Humanoid.Health = 0 end end })
T.Misc:Space()
T.Misc:Button({ Title = "Anti AFK", Desc = "Prevent auto-kick", Callback = function() if _G.YH.afkConnected then return end; _G.YH.afkConnected = true; LocalPlayer.Idled:Connect(function() VirtualInputManager:SendMouseButtonEvent(0, 0, 0, true, game, 1); task.wait(0.1); VirtualInputManager:SendMouseButtonEvent(0, 0, 0, false, game, 1) end) end })
T.Misc:Space()
...
-- ============== HUD TAB ==============
T.HUD:Toggle({ Title = "Enable HUD", Desc = "FPS, Ping, Killer info", Callback = function(s) _G.YH.hudOn = s end })
T.HUD:Space()
T.HUD:Toggle({ Title = "Show FPS", Default = true, Callback = function(s) _G.YH.hudFPS = s end })
T.HUD:Space()
T.HUD:Toggle({ Title = "Show Ping", Default = true, Callback = function(s) _G.YH.hudPing = s end })
T.HUD:Space()
T.HUD:Toggle({ Title = "Show Killer", Default = true, Callback = function(s) _G.YH.hudKiller = s end })
...
local function CreateHUD()
    ...
    local pingLbl = Instance.new("TextLabel"); pingLbl.Size = UDim2.new(1, 0, 0, 18); pingLbl.BackgroundTransparency = 1; pingLbl.Text = "Ping: 0ms"; pingLbl.TextColor3 = Color3.fromRGB(100, 200, 255); pingLbl.Font = Enum.Font.SourceSansSemibold; pingLbl.TextSize = 15; pingLbl.TextXAlignment = Enum.TextXAlignment.Left; pingLbl.Parent = frame
...
RunService.RenderStepped:Connect(function(dt)
    if _G.YH.hudOn then
        ...
        if _G.YH.hudKiller then
            local killerName = "--"
            for _, plr in pairs(Players:GetPlayers()) do if plr ~= LocalPlayer and plr.Character and plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.Health > 0 then local team = plr.Team; if team and (team.Name:lower():find("maniac") or team.Name:lower():find("killer")) then killerName = plr.Name; break end end end
    ...
    else if _G.YH.hudGui then pcall(function() _G.YH.hudGui.sg:Destroy() end); _G.YH.hudGui = nil end end
end)
...
-- ============== MAIN LOOP ==============
RunService.RenderStepped:Connect(function()
    if _G.YH.brightOn then local b = _G.YH.brightLevel * 2; Lighting.Ambient = Color3.fromRGB(255, 255, 255); Lighting.Brightness = b; Lighting.ClockTime = 12; Lighting.FogEnd = 100000; Lighting.GlobalShadows = false; Lighting.OutdoorAmbient = Color3.fromRGB(255, 255, 255); Lighting.ColorShift_Top = Color3.fromRGB(255, 255, 255); Lighting.ColorShift_Bottom = Color3.fromRGB(255, 255, 255) end
    ...
    if _G.YH.sprintOn and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then local shift = UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or UserInputService:IsKeyDown(Enum.KeyCode.RightShift); LocalPlayer.Character.Humanoid.WalkSpeed = shift and (16 * _G.YH.sprintBoost) or 16 end
end)

RunService.Heartbeat:Connect(function()
    if _G.YH.flOn then if not _G.YH.flObj then _G.YH.flObj = Instance.new("SpotLight"); _G.YH.flObj.Brightness = 2; _G.YH.flObj.Range = 60; _G.YH.flObj.Angle = 90; _G.YH.flObj.Face = Enum.NormalId.Front; _G.YH.flObj.Parent = Camera end; _G.YH.flObj.Enabled = true elseif _G.YH.flObj then _G.YH.flObj.Enabled = false end
end)